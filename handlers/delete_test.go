...
func TestDeleteExistingFile(t *testing.T) {
	...
	dataStore.InsertEntry(strings.NewReader(fileContents),
		picoshare.UploadMetadata{
			ID:       mustCreateEntryID(t, "hR87apiUCj"),
	...
	_, err := dataStore.GetEntryMetadata(mustCreateEntryID(t, "hR87apiUCj"))
	if _, ok := err.(store.EntryNotFoundError); !ok {
		t.Fatalf("expected entry %v to be deleted", mustCreateEntryID(t, "hR87apiUCj"))
	}
}
